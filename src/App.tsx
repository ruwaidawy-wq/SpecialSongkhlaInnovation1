...
const App = () => {
  ...
  const updateAward = (id, v) =>
    setSubs(subs.map((s) => (s.id === id ? { ...s, award: v } : s)));
  const reqEdit = (id, d) =>
    setSubs(
      subs.map((s) =>
        s.id === id ? { ...s, hasEditRequest: true, editData: d } : s
      )
    );
  const appEdit = (id) =>
    setSubs(
      subs.map((s) =>
        s.id === id
          ? { ...s, ...s.editData, hasEditRequest: false, editData: null }
          : s
      )
    );
  const rejEdit = (id) =>
    setSubs(
      subs.map((s) =>
        s.id === id ? { ...s, hasEditRequest: false, editData: null } : s
      )
    );
  ...
  const handleSurveySubmit = (data) =>
    setSurveys([...surveys, { id: Date.now(), ...data }]);
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1">
        ...
        {page === "survey" && (
          <SatisfactionSurvey
            onSubmit={(d) => setSurveys([...surveys, { id: Date.now(), ...d }])}
          />
        )}
      </main>
